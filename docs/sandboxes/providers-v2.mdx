...
The gateway reports the active provider mode through the workspace-readable
provider API. Workspace members can therefore use the matching provider flow
without permission to read gateway-global settings. Reading or changing the
setting itself still requires Platform Admin access.
