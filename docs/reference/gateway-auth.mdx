...
A realm per gateway also gives each gateway a distinct `issuer`, so a token from one realm is structurally unusable at another and the isolation does not depend on scope mappings being correct. The cost is a separate user set per realm; share identities through a common user federation source or by brokering each realm to a shared upstream realm. Prefer this when the gateways are separate trust domains, such as different tenants.
