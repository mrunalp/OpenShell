...
One identity provider can serve several gateways. Give each gateway its own audience and its own roles claim path so that an admin on one gateway is not an admin on the others.

The gateway derives admin from two settings, and both must be gateway-specific:

- `audience` gates authentication. The gateway rejects any token whose `aud` claim does not contain this value.
- `roles_claim` and `admin_role` gate authorization. Admin means the configured role name appears in the array at the configured claim path.

A shared audience and a shared realm role make every admin an admin everywhere, so scope both to the gateway.
...
This holds under both topologies. Use the `subject` that `openshell whoami` reports against that specific gateway, because the subject is scoped to the realm that issued the token.

#### Choosing between the two

| | One client per gateway | One realm per gateway |
|---|---|---|
| Isolation rests on | Audience plus correct scope mappings | Separate issuers and signing keys |
| Gateway settings that differ | `audience` and `roles_claim` | `issuer` only |
| Roles | Client roles on each gateway's client | Realm roles in each gateway's realm |
| Identity | One account, roles per gateway client | One account per realm, or brokered from a shared realm |
| Best for | One organization with different admin teams | Separate trust domains, such as different tenants |
