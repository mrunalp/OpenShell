...
### Multiple Gateways with One Identity Provider

One identity provider can serve several gateways. Left at its defaults, every gateway shares the `openshell-cli` audience and the realm roles `openshell-admin` and `openshell-user`, so any user who reaches a gateway URL is authorized on it and a platform admin is an admin everywhere.

Three settings can carry a per-gateway boundary, and each gives a different topology:

| Topology | Boundary | Per-gateway settings |
|---|---|---|
| Per-gateway roles | Role names, in one shared realm and client | `admin_role`, `user_role` |
| Per-gateway client | Audience and client roles | `audience`, `roles_claim` |
| Per-gateway realm | Issuer and signing keys | `issuer` |

Only the per-gateway client and per-gateway realm topologies confine a token to a single gateway. Under per-gateway roles, one token is valid at every gateway the user can reach, so a gateway that receives it can replay it against the others. Choose accordingly when gateways do not trust each other.

#### Per-gateway roles in one realm

The simplest option, and the only one that adds no clients or realms. Define two realm roles per gateway and point each gateway at its own pair:
...
admin_role  = "gw-prod-a-admin"
user_role   = "gw-prod-a-user"
```

Grant `gw-prod-a-admin` or `gw-prod-a-user` to the users who should reach that gateway. A caller holding neither is denied:

```text
role 'gw-prod-a-user' required
```

Users keep one client and one login, and share a gateway by receiving one more role. Two limits apply:

- `GetCurrentUser` and `GetGatewayConfig` deliberately require no role. Any authenticated user in the realm can call them on any gateway, whatever their gateway roles.
- The roles claim carries every role the user holds, and the token travels in a gRPC header capped at 16 KB. At roughly 22 bytes per role, a user can hold roles for a few hundred gateways before requests fail with `header list size to send violates the maximum size (16384 bytes)`. Plan for a few hundred gateways per user at most, and do not grant fleet-wide operators every gateway role.

#### Keycloak: one client per gateway

Create one client per gateway in a single realm, and define `openshell-admin` and `openshell-user` as *client* roles on each client rather than realm roles. Grant users the client role only on the gateways they administer.

Each client needs three things:

- An audience mapper (`oidc-audience-mapper`) with the client's own ID as the included audience, so `aud` identifies the gateway.
- A client-roles mapper (`oidc-usermodel-client-role-mapper`) emitting `resource_access.${client_id}.roles`, so the gateway can find the roles. Keycloak's built-in `roles` client scope maps realm roles only.
- `fullScopeAllowed` set to `false`. This is not the Keycloak default and it is required. See the warning below.

Create the client with `kcadm.sh`:

```shell
kcadm.sh create clients -r openshell -s clientId=openshell-gw-prod-a \
  -s publicClient=true -s standardFlowEnabled=true \
  -s 'attributes."pkce.code.challenge.method"=S256' \
  -s 'redirectUris=["http://127.0.0.1:*","http://localhost:*"]' \
  -s fullScopeAllowed=false

kcadm.sh create clients/$ID/protocol-mappers/models -r openshell \
  -s name=aud -s protocol=openid-connect -s protocolMapper=oidc-audience-mapper \
  -s 'config."included.client.audience"=openshell-gw-prod-a' \
  -s 'config."access.token.claim"=true'

kcadm.sh create clients/$ID/roles -r openshell -s name=openshell-admin
kcadm.sh create clients/$ID/roles -r openshell -s name=openshell-user
```

Configure the gateway against that client:

```toml
[openshell.oidc]
issuer      = "https://idp.example.com/realms/openshell"
audience    = "openshell-gw-prod-a"
roles_claim = "resource_access.openshell-gw-prod-a.roles"
admin_role  = "openshell-admin"
user_role   = "openshell-user"
```

Register the gateway with the matching client ID:

```shell
openshell gateway add https://prod-a.example.com --name prod-a \
  --oidc-issuer https://idp.example.com/realms/openshell \
  --oidc-client-id openshell-gw-prod-a \
  --oidc-audience openshell-gw-prod-a
```

The CLI stores a separate token bundle per gateway, so per-gateway clients need no extra client configuration.

Create the clients through the Admin REST API as each gateway is provisioned. A single realm import holding thousands of clients runs as one transaction and does not complete, while incremental creation handles the same volume in seconds. Client count itself is not a constraint: a realm with thousands of clients issues tokens at the same rate as a realm with one. Raise the `realms` Infinispan cache above its 10,000-entry default only if the database is remote enough that reloading client records becomes visible.

<Warning>
Set `fullScopeAllowed=false` on every gateway client. Keycloak defaults it to `true`, which places every role the user holds into the token regardless of which client requested it. Combined with the `oidc-audience-resolve-mapper` in Keycloak's built-in `roles` client scope, a token minted for any client in the realm then carries both the other gateways' client IDs in `aud` and their admin roles in `resource_access`. Any gateway in the realm accepts that token and treats the caller as a platform admin. Full scope off confines both `aud` and `resource_access` to the requesting client.
</Warning>

Verify the boundary by running `openshell whoami` against each gateway. A user who administers only `prod-a` reports `openshell-admin` there and no roles on the others.

#### Alternative: one realm per gateway

<Warning>
Realm count is the most expensive thing to scale in Keycloak. Administrative and startup cost grows sharply with the number of realms, well before client or role count becomes a factor, and large fleets require growing the realm cache alongside them. Use this topology for a handful of separate trust domains. Do not use it for one realm per user, per sandbox, or per VM — reach for per-gateway roles or per-gateway clients instead.
</Warning>

Give each gateway its own realm on the same identity provider. Each realm has its own issuer, signing keys, roles, and users, so no per-client scope tuning is involved. Only `issuer` differs between gateways; the audience and the roles claim keep their defaults and ordinary realm roles work unchanged:

```toml
[openshell.oidc]
issuer = "https://idp.example.com/realms/gateway-a"
# audience, roles_claim, admin_role, and user_role keep their defaults
```

Register each gateway against its own realm:

```shell
openshell gateway add https://prod-a.example.com --name prod-a \
  --oidc-issuer https://idp.example.com/realms/gateway-a \
  --oidc-client-id openshell-cli
```

Because realms do not share signing keys, a token from one realm fails at signature verification rather than at a claim comparison:

```text
invalid token: unknown signing key
```

The gateway rejects it before reading the issuer, audience, or roles, so no client-scope mistake can widen the boundary.

To avoid a separate credential per realm, broker each gateway realm to a shared upstream realm that holds the users, or point every realm at a common user federation source. Users then authenticate once against the upstream, and three properties still hold:

- The gateway receives a token issued by the *gateway* realm, not the upstream realm.
- Roles come from the gateway realm. Grant `openshell-admin` there, so admin stays per-gateway even though the credential is shared.
- The `sub` claim identifies the realm-local account, not the upstream user. Workspace membership is therefore per-gateway, and the same person has a different subject on each gateway.

#### Workspace membership is per-gateway

Roles are not the only boundary. Workspace membership lives in each gateway's own store, so a user who authenticates against a second gateway without an admin role there also has no workspace membership and cannot list or create sandboxes until a platform admin on that gateway adds them:

```text
not a member of workspace 'default'; ask a platform admin to run:
  openshell workspace member add --workspace 'default' --subject '<sub>' --role user
```

This holds under all three topologies. Use the `subject` that `openshell whoami` reports against that specific gateway, because the subject is scoped to the realm that issued the token.

#### Choosing a topology

| | Per-gateway roles | Per-gateway client | Per-gateway realm |
|---|---|---|---|
| Isolation rests on | Role names | Audience plus correct scope mappings | Separate issuers and signing keys |
| Gateway settings that differ | `admin_role`, `user_role` | `audience`, `roles_claim` | `issuer` |
| Keycloak objects per gateway | 2 realm roles | 1 client, 2 client roles, 1 mapper | 1 realm |
| Confines a token to one gateway | No | Yes | Yes |
| Covers the role-exempt RPCs | No | Yes | Yes |
| Per-user ceiling | A few hundred gateways | None | None |
| Scales to thousands of gateways | Yes | Yes | No |
| Identity | One account, one login | One account, roles per gateway client | One account per realm, or brokered from a shared realm |
| Best for | Many gateways inside one organization | Gateways that do not trust each other | A few separate trust domains |
